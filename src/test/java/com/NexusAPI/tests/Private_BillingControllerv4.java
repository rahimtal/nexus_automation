...
public class Private_BillingControllerv4 extends BaseClass {
	...
	@Test(priority = 4, groups = "billing", dependsOnMethods = "postcreateStatementv4")
	...
	public void putcreatebillingmessagesv4()
			throws ClassNotFoundException, SQLException, InterruptedException, IOException {
		// ExtentTest test = extent.createTest("postcreateStatementv4_isfinal");
		// test.log(Status.INFO, "Starting test: postcreateStatementv4_isfinal");
	...
	// GET Billing Account Attributes without IncludeDrillback -> server default is
	// IncludeDrillback=true (stored proc @IncludeDrillBack BIT = 1), so drillback
	// links are generated by default. This preserves existing behavior.
	...
	public void getBillingAccountAttributesDefaultv4()
		...
		Assert.assertTrue(actual.contains("\"BillingAccountAttributes\""),
				"Expected a BillingAccountAttributes response. Actual: " + actual);
		Assert.assertTrue(actual.contains("\"DrillbackLink\":\"cogsDrillback"),
				"Expected default behavior (drillback included, server default true) when IncludeDrillback is omitted. Actual: "
						+ actual);
	}
