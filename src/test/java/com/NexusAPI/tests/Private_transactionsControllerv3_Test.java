...
public class Private_transactionsControllerv3_Test  extends BaseClass{
	...
	public void getTransactions_v_3() throws ClassNotFoundException, SQLException, InterruptedException, IOException {
		...
		// Known v3 API regression: the getTransactions v3 endpoint internally enables
		// drillback and then rejects it, returning Success:false / "IncludeDrillback is
		// not allowed". The request below does not send IncludeDrillback, so there is
		// nothing to remove on the client side. Skip until the API is fixed.
