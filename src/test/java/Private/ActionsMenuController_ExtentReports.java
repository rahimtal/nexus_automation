       ...
       ////test.log(Status.INFO, "Starting test: getActionsMenu_v4");
        ...
        try {
          ...
          // //test.log(Status.INFO, "Response: " + result);
            ...
            Assert.assertEquals(result, expected, "Response does not match expected result.");
            //test.log(Status.PASS, "Response matched expected result.");
        } catch (AssertionError e) {
            //test.log(Status.FAIL, "Test failed! Response did not match expected result: " + e.getMessage());
            throw e;
        } catch (Exception e) {
            //test.log(Status.ERROR, "Unexpected error: " + e.getMessage());
            throw e;
        }
    }

   // @Test(priority = 2, groups = "ActionsMenu")
    public void anotherTestCase() throws ClassNotFoundException, SQLException, InterruptedException, IOException {
        ////ExtentTest test = extent.createTest("anotherTestCase");
        //test.log(Status.INFO, "Starting test: anotherTestCase");

        String uri = "/actionsMenu/someOtherEndpoint";
        String ver = "4.0";
        //test.log(Status.INFO, "URI: " + uri + ", Version: " + ver);

        HashMap<String, String> params = new HashMap<>();
        params.put("CustomerId", "CUSTOMER003");
        params.put("LocationId", "SEWER004");
        //test.log(Status.INFO, "Parameters: " + params.toString());
        ...
        try {
          ...
          // //test.log(Status.INFO, "Response: " + result);
            ...
            String expected = "{...}"; // Expected response JSON (truncated)
            Assert.assertEquals(result, expected, "Response does not match expected result.");
            //test.log(Status.PASS, "Response matched expected result.");
        } catch (AssertionError e) {
            //test.log(Status.FAIL, "Test failed! Response did not match expected result: " + e.getMessage());
