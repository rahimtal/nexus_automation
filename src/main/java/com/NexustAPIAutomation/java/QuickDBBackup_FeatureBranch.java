...
public class QuickDBBackup_FeatureBranch {
    ...
    public static void backupDatabase(boolean skipConfirmation) throws IOException {
        ...
        // Destination for the backup file. Defaults to the same file QuickDBRestore_FeatureBranch
        // reads from, so a backup replaces the current feature-branch snapshot.
        String backupOutputFilePath = ReadProjectProperties.ReadFile("backupOutputFilePath_FeatureBranch");
        if (backupOutputFilePath == null || backupOutputFilePath.isBlank()) {
            backupOutputFilePath = ReadProjectProperties.ReadFile("backupFilePath_FeatureBranch");
        }
